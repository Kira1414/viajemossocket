<template>
	<div>
		...
		<table class="table" id="editoriales-table" style="width:100%">
			<thead>
				<tr>
...
<script>
    ...
    export default {
        name:'ListEditorialesComponent',
        mounted() {
            this.$nextTick(() => {
                this.datatable = this.initTable()
            })

            EventBus.$on("create-editorial", e => 
            {
            ...
            EventBus.$on("delete-editorial", e => 
            {
                this.datatable.columns.adjust().draw();
            })
        ...
        methods:
        {
            initTable(){
                ...
                let table = $('#editoriales-table').DataTable({
                ...
                $("#editoriales-table tbody").on("click",".edit", function(event)
                {
                    let data = table.row($(this).parents('tr')).data();
                    vm.edit(data)
                    event.stopPropagation();
                })
                $("#editoriales-table tbody").on("click",".delete", function(event)
                {
                    let data = table.row($(this).parents('tr')).data();
                    vm.delete(data)
                    event.stopPropagation();
            ...
            deleteElement()
            {
                ...
                axios.delete(`/api/editoriales/delete/${this.id}`).then(res => {
