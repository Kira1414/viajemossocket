<template>
	<div>
		...
		<table class="table" id="libros-table" style="width:100%">
			<thead>
				<tr>
...
<script>
    ...
    export default {
        name:'ListLibrosComponent',
        mounted() {
            this.$nextTick(() => {
                this.datatable = this.initTable()
            })

            EventBus.$on("create-libro", e => 
            {
        ...
        data()
        {
            return {
                datatable: null,
                columns: ['Titulo', 'Editorial', 'Sinopsis', "# Paginas", "Acciones"],
                view: 'listar',
                id: null,
                dialogVisible: false,
            }
        ...
        methods:
        {
            initTable(){
                ...
                let table = $('#libros-table').DataTable({
                ...
                $("#libros-table tbody").on("click",".edit", function(event)
                {
                    let data = table.row($(this).parents('tr')).data();
                    vm.edit(data)
                    event.stopPropagation();
                })
                $("#libros-table tbody").on("click",".delete", function(event)
                {
                    let data = table.row($(this).parents('tr')).data();
                    vm.delete(data)
                    event.stopPropagation();
            ...
            deleteElement()
            {
                ...
                axios.delete(`/api/libros/delete/${this.id}`).then(res => {
                    this.dialogVisible = false
                    this.notify("Eliminado", "Registro eliminado", "warning")
                    this.datatable.columns.adjust().draw();
                }).catch(err => {
                    console.log(err)
                })                
